...
jQuery.widget( "mobile.scrollview", jQuery.mobile.widget, {
  options: {
    fps:               25,    // Frames per second in msecs.
    ...
    moveThreshold:     5,   // User must move this many pixels in any direction to trigger a scroll.
    moveIntervalThreshold:     150,   // Time between mousemoves must not exceed this threshold.
  
    scrollMethod:      "translate",  // "translate", "position", "scroll", use scroll as default value as it won't result in graphics artifacts and have better performance in Android.
  ...
  { 
    ...
    this._addBehaviors();
    this._setScrollPosition = this.__setScrollPosition__();
  },
  ...
  _startMScroll: function(speedX, speedY)
  {
    ...
    this.options.startEventName && this._$clip.trigger(this.options.startEventName);
  ...
  _stopMScroll: function()
  {
    if (this._timerID)
    {
      this.options.stopEventName && this._$clip.trigger(this.options.stopEventName);
  ...
  _handleMomentumScroll: function()
  {
    ...
    this.options.updateEventName && this._$clip.trigger(this.options.updateEventName, [ { x: x, y: y } ]);
  ...
  __setScrollPosition__: function()
  {
    ...
    switch (sm)
    {
      case "translate":
        return function(x, y)
        {
          ...
          this._sy = y;
          var $v = this._$view;
          setElementTransform(this._$view, x, y);
          if (this._$vScrollBar)
          {
            setElementTransform(this._$vThumb, 0, -y/this.__viewHeight__ * this._$vThumbParentHeight);
          }
          if (this._$hScrollBar)
          {
            setElementTransform(this._$hThumb, -x/this.__viewWidth__ * this._$hThumbParentWidth, 0);
          }
        };
      case "position":
        return function(x, y)
  ...
  _handleDragStart: function(e, ex, ey)
  {
    ...
    var c = this._$clip;
    var v = this._$view;
    this.__viewHeight__ = v.height();
    this.__viewWidth__ = v.width();
    ...
    if (this._hTracker)
    {
      ...
      if (this._$hScrollBar)
      {
        if (!this._$hThumb)
        {
          this._$hThumb = this._$hScrollBar.find(".ui-scrollbar-thumb");
        }
        this._$hThumbParentWidth = this._$hThumb.parent().width();
        this._$hThumb.css("width", (cw >= vw ? "100%" : Math.floor(cw/vw*100)+ "%"));
      }
    }
    ...
    if (this._vTracker)
    {
      ...
      if (this._$vScrollBar)
      {
        if (!this._$vThumb)
        {
          this._$vThumb = this._$vScrollBar.find(".ui-scrollbar-thumb");
        }
        this._$vThumbParentHeight = this._$vThumb.parent().height();
        this._$vThumb.css("height", (ch >= vh ? "100%" : Math.floor(ch/vh*100)+ "%"));
      }
    }
  ...
  {
    if (this.scrollbarVisible_) return;
...
var setElementTransform = (navigator.userAgent.search(/Mac.*Mobile/) > -1) ? function($ele, x, y)
    {
      var v = ["translate(", x, "px,", y, "px)"].join("");
      $ele.css({
        "-webkit-transform": v,
        "transform": v
      });
    } : ((navigator.userAgent.search(/Android 2/) > -1) ? function($ele, x, y)
    {
      $ele.css("-webkit-transform", ["translate3d(", x, "px,", y, "px, 0px)"].join(""));
    } : function($ele, x, y)
    {
      var v = ["translate(", x, "px,", y, "px)"].join("");
      $ele.css({
